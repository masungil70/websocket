 ...
 *   */
@ServerEndpoint("/webSocketExam/{userId}")
public class WebSocketExam {
	static private Map<Session, String> sessionMap = new HashMap<>();
	...
	public void handleOpen(Session session
		...
		// 콘솔에 접속 로그를 출력한다.
		System.out.println("client is now connected... session.getId() = " + session.getId() + "   userId = " + userId);
	...
	public void handleMessage(Session selfSession, String message)  {
		...
		for (Entry<Session, String> entry : sessionMap.entrySet()) {
  			System.out.println("sendText:" + replymessage);
  			try {
  				entry.getKey().getBasicRemote().sendText(entry.getValue() + " : " + replymessage);
  			} catch (IOException e) {
  				e.printStackTrace();
